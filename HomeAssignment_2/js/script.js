...
var audio2 = new Audio('audio/door.mp3') // Add the variable for the door open sound. 

var audio3 = new Audio('audio/meow.mp3') // Add the variable for meow sound.  
...
function mouseDown () { // Create the mouseDown function. 
	...
	if(counter == 1) {  // If the counter = 1, then image will change to 2 if the left mouse button is pressed.
		...
		audio2.play(); // Adding door opening sound. 
		audio2.volume = 0.2; // Lowering volume
...
if (counter == 0) { // Spacebar image change.
	...
	if (y == 38) { // If up arrow key is pressed, the following occurs. 
		...
		audio3.play(); // Play meow sound. This will only occur when the image is first shown, not when the mouse hover function is being used. 
